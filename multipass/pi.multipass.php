...
class Multipass {
	...
	public function Multipass()
	{
		...
		if (!$this->EE->TMPL->fetch_param('sso_key'))
		{
		...
		$link_text = $this->EE->TMPL->fetch_param('text') ? $this->EE->TMPL->fetch_param('text') : "Support";
	...
	{
		$account_key = $this->EE->TMPL->fetch_param('key'); //Tender Key
		$api_key     = $this->EE->TMPL->fetch_param('sso_key'); //Tender API Key
